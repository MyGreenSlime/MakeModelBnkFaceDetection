...
class facedetect:
    ...
    def detect(self,img):
        pic = img.copy()
        gray_img = cv2.GaussianBlur(pic,(21,21),0) #v1 = 21,21
    ...
    def predict(self,path):
        ...
        self.predicted_img1 = self.detect(self.img)
        self.resize = cv2.resize(self.predicted_img1, (3*int(self.img.shape[1]/4),3*int(self.img.shape[0]/4)), interpolation=cv2.INTER_CUBIC)
        cv2.imshow('FaceDetector', self.resize)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
model = facedetect()
model.predict('./test-data/ptal.jpg')